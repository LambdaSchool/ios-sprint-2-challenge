...
class CrayonHelper {
    ...
    // Group color names by letter, ex: "A": ["Apple Red", "Augmented Pink", "Autumn Brown" ...]
    private var sectionDictionary: [String: [String]] = [:]
    
    // Sorted collection of color letter indices - entire list of keys is put into sectionKeys array
    ...
    private init() {
        for (name, color) in crayonDictionary {
            // grab uppercase name and pull the first letter
    ...
    // Section name lookup - First letter
    ...
    // Retrieve a crayon record according to index path
